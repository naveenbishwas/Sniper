...
export default function Header({ onSignupClick }) {
  ...
  // ---- If email exists but uniqueId not found, fetch from Firestore
  useEffect(() => {
    const fetchIdByEmail = async () => {
      if (!user?.email || uniqueId) return;
  ...
  return (
    ...
    >
      ...
      {/* RIGHT SIDE */}
      <div className="header__right">
        {/* ✅ If no logged-in user, show both buttons */}
        {!initial ? (
          <>
            <button
              className="header__signin"
          ...
          /* ✅ If user logged in, show avatar with dropdown */
          <div className="account-wrap">
            <button
              id="avatar"
              className="user-avatar"
              ...
              <div className="account-popover" role="dialog" ref={popRef}>
                ...
                <div className="acc-footer">
                  <button
                    ...
                    className="acc-signout"
                    onClick={() => {
                      ["fullName", "email", "userRole", "uniqueId"].forEach(
